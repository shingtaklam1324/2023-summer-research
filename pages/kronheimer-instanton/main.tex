...
be the ``trivial'' solution to Nahm's equations. We are interested in solutions which are asymptotic to this one. In \cite[Proof of Theorem 1, p. 482]{kronheimer_instantons_1990}, we have exponential convergence of \(A\), i.e.

\[\abs{A - \rho} \le Ke^{\eta t}\]

for some \(\eta > 0\), as \(t \to -\infty\). This then gives us that in the limit as \(s \to -\infty\),

\[\abs{B - B^0} \le \frac{Ke^{\eta t}}{\abs{s}} = \frac{K}{\abs{s}^{1+\zeta}} \le \frac{C}{(1-s)^{1+\zeta}}\]

for some \(C, \zeta > 0\). A similar proof for the other boundary condition gives us that

\[\abs{B} \le \frac{C}{\abs{s}^{1+\zeta}}\]

for some \(C, \zeta > 0\), although we won't use this result in what follows. Let \(\Omega_1\) denote the space of all \(C^1\) maps
...
This immediately gives us the bound on the second derivative. The same argument as in the previous subsection shows that this gives us a bound on \((1-s)^{1+\delta}\dot u\). Therefore, all we need is a bound on \((1-s)^{2+\delta}u\). Defining \(h(s) = (1-s)^{2+\delta_0}\), and using the same argument again, we get a bound on \(h\).
