...
Let \(M\) be a manifold. In this section, we will define what a K\"ahler structure on \(M\) is.
...
Therefore, we must then have that \(\ad_X(B) = 0\).
...
\[g_A(e_{ij}, e_{ij}) = g_A(f_{ij}, f_{ij}) = 2(\lambda_j - \lambda_i)\]
...
\[\omega_A(e_{ij}, f_{ij}) = 2(\lambda_j - \lambda_i) = g_A(f_{ij}, f_{ij}) = g_A(J_A(e_{ij}), f_{ij})\]
...
where the last line is because we require \(i < j\) and \(k < l\).
...
Therefore, the Riemannian metric is given by

\[\iinner{X, Y}_B = \iinner{\Ad_{a^*}(X), \Ad_{a^*}(Y)}_A\]

\section{\(\SL(n, \C)\) orbits}

First we will sketch how to generalise what we have shown so far to \(\SL(n, \C)\). The same formulae as in \cref{sec:adjoint} can be used to define the adjoint and coadjoint representations of \(\SL(n, \C)\), and the same inner product can be used to show that the two are isomorphic.
